...
const initialTripData = {
  ...
  days: [
    ...
    },
    {
      ...
      events: [
        { 
          id: "d3-1", time: "09:00", title: "半潛艇（玻璃船）", 
          location: "南寮漁港右道登船口第6根柱子", phone: "0912151471", type: "spot", 
        ...
        { id: "d3-8", time: "17:30", title: "沿途順遊", location: "北迴歸線地標 台11線", type: "spot" },
    ...
    },
    {
      ...
      events: [
        { id: "d5-1", time: "09:30", title: "前往烏石港", location: "烏石港漁會大樓", type: "drive", note: "建議提早出發，10:00前需抵達報到" },
        { 
          id: "d5-2", time: "10:30", title: "宜蘭龜山島登島賞鯨半日遊 (二合一)", 
          location: "261台灣宜蘭縣頭城鎮烏石港路168號 (1樓2號櫃台)", phone: "0911217567", type: "spot", 
          note: "1大1小。10:30 航班，請於 10:00 前完成報到",
          remarks: [
            "⚠️ 必帶：身分證或具照片之身分證件以供查驗",
            "建議攜帶防曬油、太陽眼鏡，並於上船前30分鐘吃暈船藥",
            "現場需另付登島清潔維護費：每人 100 元",
            "前一晚 18:00 前，請主動加 Line(0963499016) 聯繫確認",
            "若遇天候不佳，將於出發前一天 20:00 決定是否取消出發",
            "龜山島上未開放龜尾湖下水游泳玩水，請勿擅自下水"
          ]
        },
        { id: "d5-3", time: "14:00", title: "烏石港吃午餐", location: "烏石港", type: "spot" },
        { id: "d5-4", time: "15:30", title: "宜蘭景點", location: "羅東林業文化園區", type: "spot", note: "下午彈性安排景點" },
...
export default function MyTrip() {
  ...
  useEffect(() => {
    if (isMounted) {
      localStorage.setItem('roadtrip-ai-v3-data', JSON.stringify(trip));
    }
  }, [trip, isMounted]);
                      ...
                      <div className="bg-[#FFF8E7] p-3 rounded-lg border-2 border-dashed border-[#D9B48F] space-y-1.5 mt-2">
                        ...
                        <ul className="list-disc list-inside text-xs space-y-1 text-gray-700 font-medium leading-relaxed">
                          {event.remarks.map((rem, rIdx) => (
                            <li key={rIdx} className={rem.includes('⚠️') || rem.includes('不退費') || rem.includes('必帶') ? 'text-red-600 font-bold' : ''}>
                              {rem}
                            </li>
                          ))}
                        </ul>
                          ...
                          <div key={suggestion.id} className="flex flex-col bg-[#FDF9F1] p-3 rounded-lg border border-[#8D6E63] space-y-2">
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-xs text-[#8D6E63] font-bold mt-1">{suggestion.note}</p>
                              </div>
                              <button onClick={() => addSuggestedEvent(suggestion)} className="flex items-center gap-1 bg-white border-2 border-black px-2 py-1 rounded-lg text-xs font-bold shadow-[2px_2px_0px_black] active:translate-y-0.5 active:shadow-[0px_0px_0px_black] flex-shrink-0">
                                <PlusCircle size={14}/> 加入
                              </button>
                            </div>
                            {suggestion.remarks && suggestion.remarks.length > 0 && (
                               <div className="text-xs bg-[#FFF8E7] p-2 rounded border border-dashed border-[#D9B48F] text-gray-600">
                                 ...
                                 <ul className="list-disc list-inside">
                                   {suggestion.remarks.map((rem: string, idx: number) => <li key={idx}>{rem}</li>)}
                                 </ul>
                               </div>
                            )}
                          </div>
